...
//    saveChecklistItems()
  ...
  func configureText(for cell: UITableViewCell, with item: ChecklistItem) {
    // Настраиваем ячейку для отображения itemID и наименования задачи
    let label = cell.viewWithTag(1000) as! UILabel
    label.text = "\(item.text)"
    label.lineBreakMode = .byWordWrapping
    label.numberOfLines = 0
    label.sizeToFit()
    // Настраиваем ячейку для отображения даты напоминания задачи
    let dueDateLabel = cell.viewWithTag(2000) as! UILabel
    
    let date = item.dueDate // Nov 10, 9:44 PM
    let dateFormatter = DateFormatter()
    dateFormatter.locale = Locale(identifier: "ru-Ru")
    dateFormatter.dateFormat = "dd-MM-YYYY HH:mm"
    let dateString = dateFormatter.string(from: date) // "2015-11-10 09:44 PM"
    
    dueDateLabel.text = "Дата: \(dateString)"
    dueDateLabel.lineBreakMode = .byWordWrapping
